...
(require "hw2-2.rkt") ; change "hw1-1.rkt" to your filename
...
(define (mobileGrade)
  (begin
    ...
    (sgoutput (lambda () (equal? 3 (weight mbl1)) ))
    (sgoutput (lambda () (equal? 10 (weight mbl2)) ))
    (sgoutput (lambda () (equal? 13 (weight mbl3)) ))
    (sgoutput (lambda () (equal? #t (is-balanced? mdl1)) ))          
    (sgoutput (lambda () (equal? #f (is-balanced? mbl1)) ))
    (sgoutput (lambda () (equal? #t (is-balanced? mbl2)) ))
    (sgoutput (lambda () (equal? #f (is-balanced? mbl3)) ))
    (sgoutput (lambda () (equal? #t (is-balanced? (make-mobile b6 (make-branch 6 (model 5))))) ))
))
