...
(provide init-maze open-s open-n open-e open-w maze-pp)
...
(define (init-maze n m)
  ...
  (cons (cons n m) (make-list (make-list (make-list #f 4) m) n)))
...
(define (open i j z maze)
  ...
  (define m (cdar maze))
  (define lst (cdr maze))
  (if (and (<= 0 i) (< i n) (<= 0 j) (< j m) (<= 0 z) (< z 4))
      (cons (cons n m) (change-list3 i j z lst #t))
      maze))
      
(define (open-n i j maze) (open (- i 1) j 2 (open i j 0 maze)))
(define (open-w i j maze) (open i (- j 1) 3 (open i j 1 maze)))
(define (open-s i j maze) (open (+ i 1) j 0 (open i j 2 maze)))
(define (open-e i j maze) (open i (+ j 1) 1 (open i j 3 maze)))
...
(define (maze-pp maze)
  ...
  (define (draw co)
    ...
    (begin
      (if (list-ref elem 0) (void) (draw-line x y sz 0 0 0))
      (if (list-ref elem 1) (void) (draw-line x y 0 0 0 sz))
      (if (list-ref elem 2) (void) (draw-line x y 0 sz sz sz))
      (if (list-ref elem 3) (void) (draw-line x y sz sz sz 0))
    ))
  (begin
    (display "%!PS\n" file)
